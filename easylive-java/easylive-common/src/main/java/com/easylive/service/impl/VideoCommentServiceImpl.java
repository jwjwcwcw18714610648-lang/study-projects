...
public class VideoCommentServiceImpl implements VideoCommentService {
	...
	VideoComment videoComment=videoCommentMapper.selectByCommentId(commentId);//查对应id的评论
		...
		VideoInfo videoInfo= (VideoInfo) videoInfoMapper.selectByVideoId(videoComment.getVideoId());//查到对应评论的所处在的视频信息
	...
	public void deleteComment(Integer commentId, String userId) {
		...
		VideoInfo videoInfo= (VideoInfo) videoInfoMapper.selectByVideoId(videoComment.getVideoId());//查询视频信息
	...
	if(!userId.equals(videoInfo.getUserId())&&!userId.equals(videoComment.getUserId()))throw new BusinessException(ResponseCodeEnum.CODE_600);
		...
		if(videoComment.getpCommentId()=='0'){//说明其为父评论
			videoInfoMapper.updateCountInfo(videoInfo.getVideoId(),UserActionTypeEnum.VIDEO_COMMENT.getField(),-1);//视频评论数据减去1
			VideoCommentQuery videoCommentQuery=new VideoCommentQuery();
	...
	public void postComment(VideoComment comment, Integer replyCommentId) {
		...
		VideoInfo videoInfo = (VideoInfo) videoInfoMapper.selectByVideoId(comment.getVideoId());
		if (videoInfo == null) {
			throw new BusinessException(ResponseCodeEnum.CODE_600);
		}
		//是否关闭评论
		...
		if (replyCommentId != null) {
			...
			UserInfo userInfo = (UserInfo) userInfoMapper.selectByUserId(replyComment.getUserId());//查上级评论的 信息 昵称与头像
		...
		if (comment.getpCommentId() == 0) {
			this.videoInfoMapper.updateCountInfo(comment.getVideoId(), UserActionTypeEnum.VIDEO_COMMENT.getField(), 1);//只增加父id为0的评论数
		}
	}
}